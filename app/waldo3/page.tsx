...
export default function Home() {
    ...
    const FinishGame = async () => {
        const { data: sessionData, error: fetchError } = await supabase.from('current_session_3').select('seconds').eq('player_name', playerName)
        const seconds = sessionData?.[0].seconds
        const { data, error } = await supabase.from('Leaderboard_Image_3').insert([{ name: playerName, seconds: seconds }])
        navigate()
    }
    useEffect(() => {
    ...
    const handleSubmit: MouseEventHandler<HTMLButtonElement> | undefined = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        ...
        if (insertError) {
            ...
            localStorage.setItem('image_no', '3')
        }
    }
    ...
    const handleWaldoClick: MouseEventHandler<HTMLImageElement> | undefined = async (e: React.MouseEvent<HTMLImageElement, MouseEvent>) => {
        ...
        if (X_CP.includes(percentage.x) && Y_CP.includes(percentage.y)) {
            arr.push('waldo')
            const { error: updateError } = await supabase.from('current_session_3').update({ character_completed: arr }).eq('player_name', playerName);
            setFoundWaldo(true);
        }
        else {
            return (
        ...
        const { data: currentSessionData, error: currentError } = await supabase.from('current_session_3').select().eq('player_name', playerName);
                ...
                isLoading
                    ...
                    <div className="flex flex-col items-center justify-start gap-5 w-5/6">
                        <div className="flex flex-row justify-start items-center  w-[100%] bg-black text-white rounded-lg  p-4 gap-4 ">
                            <Counter image_no={3} player_name={playerName} stop={stop} />
