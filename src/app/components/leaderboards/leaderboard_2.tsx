...
export default function Leaderboard1(){
    ...
    useEffect(()=> {
        const fetchData = async () => {
            const { data: lb2Data , error: fetchError } = await supabase.from('Leaderboard_Image_2').select('*').order('seconds' , {ascending: true})
            setData(lb2Data)
        }
        fetchData()
        return () => setData([])
    ...
    return (
        <div className=" p-4">
            <h1 className="text-3xl font-bold mt-4">Warground</h1>
